...
def plot_variances(all_variances, values, var_type='Regular'):
    ...
    for bin_num in range(1, 13):
        ...
        # Add baseline to each subplot but only show in legend once
        show_legend_baseline = (bin_num == 1)  # Ensure baseline legend is shown only once
        fig.add_trace(go.Scatter(
            x=x_values,
            y=[0] * len(x_values),
            mode='lines',
            ...
            showlegend=show_legend_baseline  # Show baseline in legend only in the first subplot
        ), row=row, col=col)

    fig.update_layout(
        title=f'{var_type} Difference from Initial Variance for All Bins over Iterations (k > 510)',
    ...
    py.plot(fig, filename=f'Siddharth Decay/Siddharth Decay Charts/12bins_var_difference_subplots_kge510.html')
