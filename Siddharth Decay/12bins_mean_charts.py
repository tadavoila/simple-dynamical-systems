...
def load_and_process_data(k_str, runs):
    """Load data for a given k_str and return the means for each of the 12 bins separately."""
    ...
    for idx, (k_str, means) in enumerate(tqdm(all_means.items(), desc='Plotting means')):
        ...
        for bin_num in range(1, 13):
            ...
            fig.add_trace(go.Scatter(
                x=x_values,
                y=pd.concat(means[f'means_{bin_num}_new'], axis=1).mean(axis=1)[:20000],
                mode='lines',
                name=f'k = {value_used}',
...
if __name__ == '__main__':
    ...
    for k_str in tqdm(k_strings, desc='Loading and processing data'):
        means = load_and_process_data(k_str, runs)
        all_means[k_str] = means
