...
def load_and_process_data(k_str, runs):
    ...
    for run_number in tqdm(range(runs), desc=f'Processing runs for k={k_str}'):
        ...
        for i in range(1, 13):
            all_variances[f'alt_variances_{i}_new'].append(pd.Series(data[f'alt_variances_{i}_new']))
...
def plot_variances(all_variances, values, var_type='Regular'):
    ...
    for bin_num in range(1, 13):
        ...
        for idx, (k_str, variances) in enumerate(tqdm(all_variances.items(), desc='Plotting variances')):
            ...
            fig.add_trace(go.Scatter(
                x=x_values,
                y=pd.concat(variances[f'alt_variances_{bin_num}_new'], axis=1).mean(axis=1)[:20000],
                mode='lines',
                name=f'k = {value_used}',
    ...
    fig.update_layout(
        title=f'Alternate {var_type} for All Bins over Iterations (k >= 110)',
    ...
    py.plot(fig, filename=f'Siddharth Decay/Siddharth Decay Charts/alt_12bins_{var_type.lower()}_subplots_kge110.html')
