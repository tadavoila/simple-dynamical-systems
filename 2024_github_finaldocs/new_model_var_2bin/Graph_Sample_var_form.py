...
csv_paths = [f'averages_run{i}.csv' for i in range(10)]
...
combined_df = process_csv_files(csv_paths)

# Extract iteration numbers, average values, and variances
x = combined_df.index + 1
y1 = combined_df['FREQ_1_TO_6']
y2 = combined_df['FREQ_7_TO_12']
variance1 = combined_df['VAR_1_TO_6']
variance2 = combined_df['VAR_7_TO_12']
...
# Add the variances
fig.add_trace(go.Scatter(x=x, y=variance1, mode='markers', name='Variance Low Frequency (1-6)', marker=dict(color='lightcoral')))
fig.add_trace(go.Scatter(x=x, y=variance2, mode='markers', name='Variance High Frequency (7-12)', marker=dict(color='lightblue')))

# Update layout
fig.update_layout(
    title='Group Averages vs. Iteration (Combined Runs) with Difference Curve and Variances',
    xaxis_title='Iteration',
    yaxis_title='Average Value / Difference / Variance',
)
