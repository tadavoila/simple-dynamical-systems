...
data = {f'FREQ_{i}_AVG': {} for i in range(1, 13)}
data.update({f'FREQ_{i}_VAR': {} for i in range(1, 13)})  # Include variance columns
decay_rates_set = set()
...
# Initialize a dictionary to store the data
data = {f'FREQ_{i}_VAR': {} for i in range(1, 13)}  # Only include variance columns
decay_rates_set = set()
    ...
    if match:
        ...
        for bin_name in data.keys():
            if bin_name in df.columns:
                ...
                # Append the variance to the list for its decay rate
                #Recalcute this value


                
                data[bin_name][decay_rate].append(df[bin_name] )
                #data[bin_name][decay_rate].append(df[bin_name] - (first value) )
...
# Plot each frequency bin as a separate line
for bin_name, variances in data.items():
    # Prepare the y values sorted according to the decay rates
    y_values = [variances[k] for k in decay_rates]
    
    fig.add_trace(go.Scatter(x=transformed_k_values, y=y_values, mode='lines+markers', name=bin_name))

fig.update_layout(
    title='Overall Average Variances for Frequency Bins vs. Transformed Decay Rate',
    xaxis_title='Transformed K Value (1- (1/k))',
    yaxis_title='Overall Average Variance',
    legend_title='Frequency Bin'
)


overall_avg_variances = {decay_rate: np.mean([data[bin_name][decay_rate] for bin_name in data.keys()]) for decay_rate in decay_rates}

# Add the overall average line to the plot
fig.add_trace(go.Scatter(x=transformed_k_values, y=list(overall_avg_variances.values()), mode='lines', name='Overall Average', line=dict(color='black')))
