...
k_str = '95935' #If last digit is 0, do not include it
...
def database(runs):    
    ...
    for a in range(1, runs+1):
        means_1_6_path = 'Outputs/Old/old_means_1_6_r' + str(a) + f"_k{k_str}" + '.json'
        means_7_12_path = 'Outputs/Old/old_means_7_12_r' + str(a) + f"_k{k_str}" + '.json'
...
def plots(runs):
    ...
    # Update the layout
    fig.update_layout(title=f'Old Model average exemplar means over time (averaged per 200 iterations), k = 0.{k_str}', 
...
def sw_database(runs):    
    # Initialize lists to store dataframes for each group
    dfs_1_6 = []
    dfs_7_12 = []
    
    for a in range(1, runs+1):
        means_1_6_path = 'Outputs/Old/sw_old_means_1_6_r' + str(a) + f"_k{k_str}" + '.json'
        means_7_12_path = 'Outputs/Old/sw_old_means_7_12_r' + str(a) + f"_k{k_str}" + '.json'
...
def sw_plots(runs):
    df_1_6, df_7_12 = sw_database(runs)
    ...
    # Update the layout
    fig.update_layout(title=f'Old Model average exemplar strength-weighted means over time (averaged per 200 iterations), k = 0.{k_str}', 
                      xaxis_title='Iteration', 
                      yaxis_title='Mean',
                      plot_bgcolor='rgba(0,0,0,0)',
                      font=dict(
